...
@section('content')
    ...
    <div class="flex flex-row justify-center text-center items-center py-5">
      ...
      <h1 class="text-3xl text-center ml-8 font-bold"> Edit Portafolio</h1>
        ...
        <div class="mb-5">
          ...
          <select id="categoria_id" class="block appearance-none border border-gray-200 
            ...
            @foreach ($categorias as $categoria)
                  <option
                      {{ $portafolio->categoria_id == $categoria->id ? 'selected' : ''}} 
                      value="{{ $portafolio->categoria_id }}">
                      {{ $categoria->nombre }}
                  </option>
              @endforeach
          </select>
      ...
      <div class="mb-5">
          ...
          <select id="habilidad_id" class="block appearance-none border border-gray-200 
             ...
             @foreach ($habilidads as $habilidad)
                  <option
                      {{ $portafolio->habilidad_id == $habilidad->id ? 'selected' : ''}} 
                      value="{{ $portafolio->habilidad_id }}">
                      {{ $habilidad->nombre }}
                  </option>
        ...
        <div class="mb-5">
          ...
          <input name="imagen_portafolio" value="{{old('imagen_portafolio')}}" class="block w-full text-sm text-black border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-100 focus:outline-none dark:bg-blue-500 dark:border-blue-600 dark:placeholder-gray-50" aria-describedby="file_input_help" id="file_input" type="file">
         
      
         <img style="width:200px; margin-top:20px;" src="/storage/{{ $portafolio->imagen_portafolio}}">
